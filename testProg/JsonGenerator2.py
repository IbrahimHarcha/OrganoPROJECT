...
class Widget1():
    ...
    def gui(self, parent):
        ...
        self.label1.place(x = 110, y = 20, width = 400, height = 82)
        ...
        self.test.place(x = 30, y = 420, width = 310, height = 30)
        self.test['command']=self.testJson

    def testJson(self):         
        cmt1=0      # Ces deux valeurs servent de repère, afin de lancer le bon type d'erreur, on aurait pu aussi prendre des booléeens..
        cmt2=0
        if(self.nbElement>0):
            for j in self.data["possibilites"]:
                for k in self.li:
                    if(j[str(k)]=="vide" or j[str(k)]=="" or j[str(k)]==None):
                        cmt1=1
                if(self.data["possibilites"].count(j)>1): # Si jamais le nombre d'occurence de j est supèrieure a deux càd présent 2 fois alors on déclange un msg d'erreur
                    cmt2=1
            if(cmt1==1):
    ...
    def backObj(self): # Cette fonction permet de retourner en arrière en soustrayant 1 à self.var...
        if(self.var!=0):
            ...
            self.backObjet.destroy() # Si self.var est à 0 alors impossible de revenir en arrière...
    ...
    def completeObj(self):
        ...
        if(self.var<self.nbElement): # On est loins de déborder sur les indices...
            ...
            self.cplObj.place(x = 300, y = 380, width = 200, height = 30)
            self.cplObj['command']=self.completeObj
        if(self.var>0): # Il est maintenant possible de pouvoir revenir en arrière car cela signifie qu'on a déjà compléter/initialisé self.data["possibilites"][0]
            ...
            self.backObjet.place(x = 70, y=380, width = 200, height = 30)
